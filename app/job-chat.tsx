...
import { chatMessages, workerProfiles, ChatMessage } from '../constants/mockData';
  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => (
    <View style={[styles.msgWrapper, item.isMe ? styles.msgWrapperRight : styles.msgWrapperLeft]}>
      ...
      <View style={[styles.bubble, item.isMe ? styles.bubbleMe : styles.bubbleThem]}>
        <Text style={[styles.bubbleText, item.isMe && styles.bubbleTextMe]}>{item.content}</Text>
        <Text style={[styles.bubbleTime, item.isMe && { color: 'rgba(255,255,255,0.7)' }]}>
          {item.timestamp}
        </Text>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  chatNoticeText: { fontSize: 11, color: Colors.textMuted, textAlign: 'center', fontStyle: 'italic' },
  messagesList: { padding: Spacing.md, paddingBottom: 16 },
  msgWrapper: { flexDirection: 'row', alignItems: 'flex-end', gap: 8, marginBottom: 8 },
  ...
  bubble: { maxWidth: '72%', borderRadius: Radius.md, padding: 10, paddingHorizontal: 14, gap: 4 },
  ...
  textInput: {
    ...
    fontSize: 14, color: Colors.textPrimary, maxHeight: 100, backgroundColor: Colors.offWhite,
  },
  sendBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.primary, justifyContent: 'center', alignItems: 'center' },
  sendBtnDisabled: { backgroundColor: Colors.textMuted },
  sendBtnText: { color: Colors.white, fontSize: 24, fontWeight: '700', lineHeight: 28 },
});
